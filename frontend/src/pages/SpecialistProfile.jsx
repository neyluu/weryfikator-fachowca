...
export default function SpecialistProfile() {
    ...
    const fetchRatings = async () => {
        try {
            ...
            const res = await fetch(`/api/ratings/specialist/${id}`, { method: "GET", headers });
            if (res.ok) {
    ...
    useEffect(() => {
        async function fetchProfileData() {
            try {
                ...
                const profileRes = await fetch(`/api/profile/${id}`, { method: "GET", headers });

                if (!profileRes.ok) {
                    if (profileRes.status === 404) throw new Error("Nie znaleziono profilu fachowca.");
    ...
    const handleRatingSubmit = async (e) => {
        ...
        if (!scores.quality || !scores.price || !scores.timeliness) {
            setSubmitError("Musisz ocenić wszystkie trzy kategorie (Jakość, Cena, Terminowość).");
            return;
        }
            ...
            <Section title="Opinie użytkowników">
                {ratings.length > 0 ? (
                        ...
                        {ratings.map((ratingItem) => {
                            const avgScore =
                                ratingItem.quality && ratingItem.price && ratingItem.timeliness
                                    ? Math.round((ratingItem.quality + ratingItem.price + ratingItem.timeliness) / 3)
                                    : ratingItem.score || 0;
                                ...
                                <Card key={ratingItem.id} className="flex flex-col gap-5">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                        <div className="flex items-center gap-2">
                                            <Avatar name={ratingItem.authorName ? ratingItem.authorName.charAt(0).toUpperCase() : "U"} />
                                        ...
                                        <div className="flex flex-col items-start sm:items-end gap-1">
                                            ...
                                            {(ratingItem.quality || ratingItem.price || ratingItem.timeliness) && (
                                                <div className="flex gap-3 text-[10px] text-neutral-500 font-medium mt-1">
                                                    {ratingItem.quality && <span>Jakość: {ratingItem.quality}</span>}
                                                    {ratingItem.price && <span>Cena: {ratingItem.price}</span>}
                                                    {ratingItem.timeliness && <span>Terminowość: {ratingItem.timeliness}</span>}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="p-6 text-center border border-neutral-800 rounded-3xl bg-neutral-900/50">
                        <p className="text-neutral-500">Ten fachowiec nie ma jeszcze żadnych opinii.</p>
                    </div>
                )}

                {user ? (
                    <Card className="mt-6 flex flex-col gap-4 border-dashed border-2 border-neutral-700/50 bg-neutral-900/20">
                        <h3 className="text-lg font-medium text-neutral-100">Dodaj opinię</h3>
                    ...
                    <div className="mt-6 p-6 text-center border border-neutral-800 rounded-3xl bg-neutral-900/50 flex flex-col items-center gap-3">
                        <p className="text-neutral-400">Zaloguj się, aby podzielić się swoją opinią o tym fachowcu.</p>
                        <Button href="/auth/login" look="secondary">Zaloguj się</Button>
                    </div>
                )}
            </Section>
        </div>
